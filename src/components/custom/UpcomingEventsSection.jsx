...
export default function UpcomingEventsSection() {
  ...
  const upcomingEvents = useMemo(() => {
    ...
    return events
      .filter((event) => {
        if ((!event.eventDate && !event.fullDate) || event.fullDate === 'TBD') {
          return false
        }

        const dateStr = event.eventDate || event.fullDate
        const eventDateObj = new Date(dateStr)
        const eventDayStart = new Date(eventDateObj)
        eventDayStart.setHours(0, 0, 0, 0)

        // STRICTLY FUTURE: Event Date > Today
        return eventDayStart.getTime() > today.getTime()
      })
      .map((event) => {
        const dateStr = event.eventDate || event.fullDate
        return { ...event, dateObj: new Date(dateStr) }
      })
      .sort((a, b) => a.dateObj - b.dateObj)
      .slice(0, 3)
  }, [events])

  // --- HOOK 2: Create Categories ---
  ...
  // --- HOOK 4: Effect for default selection ---
  useEffect(() => {
    if (eventCategories.length > 0) {
      if (!selectedCategory || !eventCategories.find(c => c.id === selectedCategory)) {
        setSelectedCategory(eventCategories[0].id)
      }
    }
  }, [eventCategories, selectedCategory])

  // --- HOOK 5: Get Current Event ---
  ...
  // Double check to ensure currentEvent exists before rendering
  ...
  return (
    <div className="min-h-[50vh] w-full bg-[#171A23] py-12">
      <div className="max-w-6xl mx-auto px-8 min-[450px]:px-16 sm:px-20 lg:px-28">
        ...
        {/* CATEGORY TABS */}
        <div className="grid max-[450px]:grid-cols-2 grid-cols-3 gap-3 sm:gap-4 mb-8">
              ...
              <button
                ...
                className={`
                  ...
                  ${isActive
                    ...
                    : 'bg-[#1E222D]/50 border-white/10 hover:border-[#12dc9f]/50'
                  }
        ...
        <div className="relative rounded-2xl overflow-hidden backdrop-blur-sm">
          <div className="flex flex-col sm:flex-row items-center justify-center">
            <div className="relative py-6 sm:py-8 md:py-12 px-0 w-full sm:w-1/2 md:w-3/5">

              {/* Badges Container */}
              <div className="flex flex-wrap gap-2 mb-4">
                {currentEvent.isClosingSoon && (
                  <div className="inline-flex items-center gap-1 sm:gap-2 px-2 md:px-4 py-2 rounded-full bg-amber-500/20 border border-[#f59e0b]/50">
              ...
              {/* Mobile Image */}
              <div className="w-full pb-4 sm:hidden flex items-center justify-center">
                <ImagePlaceholder imgSrc={currentEvent.imgSrc} alt={currentEvent.title} />
              </div>

              {/* Description */}
              <p className="text-gray-300 text-sm md:text-base mb-8 max-w-lg leading-relaxed line-clamp-3">
              ...
              <div className="flex items-center gap-2 text-[#369fff] mb-8">
                ...
                <span className="text-sm sm:text-base font-medium">
                  {currentEvent.dateObj.toLocaleDateString('en-US', {
                    month: 'long',
                    day: 'numeric',
                    year: 'numeric',
                  })}
            ...
            {/* Desktop Image */}
            <div className="hidden sm:flex sm:w-1/2 md:w-2/5 justify-center lg:justify-end">
              <ImagePlaceholder imgSrc={currentEvent.imgSrc} alt={currentEvent.title} />
            </div>
          </div>
        </div>
