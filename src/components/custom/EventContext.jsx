...
import { fetchData } from '@/config/event_appwrite'
...
function calculateStatus(event) {
  ...
  const eventOccurrenceDate = rawEventDate
    ? new Date(rawEventDate)
    : (event.fullDate && event.fullDate !== 'TBD' ? new Date(event.fullDate) : null)
  ...
  if (eventOccurrenceDate && !isNaN(eventOccurrenceDate.getTime())) {
    displayDay = eventOccurrenceDate.getDate().toString()
  ...
  if (regClosingDate && !isNaN(regClosingDate.getTime()) && hasFormLink) {
    const timeDiff = regClosingDate.getTime() - now.getTime()
    const daysDiff = Math.ceil(timeDiff / (1000 * 3600 * 24))

    // Open: Now is before Closing Date
    isApplicationOpen = now < regClosingDate

    // Closing Soon: Within 2 days
    isClosingSoon = daysDiff <= 2 && daysDiff > 0

    // Closed: Now is AFTER Closing Date, but BEFORE Event Date
    // (If it's after Event Date, it becomes "isCompleted")
    isRegistrationClosed = now > regClosingDate && now < eventOccurrenceDate
  }

  const isRunning = now.toDateString() === eventOccurrenceDate.toDateString()
  const isCompleted = now > eventOccurrenceDate && !isRunning
  ...
  useEffect(() => {
    const syncData = async () => {
      try {
        ...
        const mergedEvents = rawEvents.map((localEvent) => {
          const remoteEvent = documents.find((doc) => {
            if (!doc.title) return false;
            return doc.title.trim().toLowerCase() === localEvent.title.trim().toLowerCase()
          })

          if (remoteEvent) {
            return calculateStatus({
              ...localEvent,
              formLink: remoteEvent.registrationForm,
              registrationClosingDate: remoteEvent.registrationClosingDate,
              eventDate: remoteEvent.eventDate,
              appwriteId: remoteEvent.$id,
            })
          }
          return calculateStatus(localEvent)
        })

        setEvents(mergedEvents)
      } catch (error) {
        console.error('Error syncing events:', error)
        setEvents(rawEvents.map(calculateStatus))
      } finally {
        setLoading(false)
      }
    }
